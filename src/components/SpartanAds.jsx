import React from 'react'


export const SpartanAds = () => {
  return (
    <div className="card--container">
      <div className="card">
        <div className="b--ads"> Ads by Fleet Labs </div>
         <h2> Learn to Build Web Apps </h2>
        <p> 
          Digital products are very easy to build. Most people just don’t have the right ideas. 
          Get my FULL GUIDE teaching you how to make ¢2,500/m+ in a few weeks.
        </p>

        <a className="join--button" href="https://t.me/+oCeVAiPJM482NjI0" rel="noreferrer" target="_blank"> Get it for only ¢150 </a>
      </div>
    </div>
  )
}
